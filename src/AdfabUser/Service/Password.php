...
use AdfabUser\Mapper\UserInterface as UserMapperInterface;
use AdfabUser\Mapper\Password as PasswordMapper;
...
class Password extends EventProvider implements ServiceManagerAwareInterface
{
    ...
    public function sendForgotEmailMessage($to, $model)
    {
        ...
        $message = $mailService->createHtmlMessage($from, $to, $subject, 'adfab-user/frontend/email/forgot', array('record' => $model, 'to' => $to, 'skinUrl' => $skinUrl));
    ...
    public function getPasswordMapper()
    {
        if (null === $this->passwordMapper) {
            $this->setPasswordMapper($this->getServiceManager()->get('adfabuser_password_mapper'));
        }
