...
class User extends \ZfcUser\Service\User implements ServiceManagerAwareInterface
{
    ...
    public function register(array $data)
    {
		...
		*/
        ...
        if ($this->getOptions()->getEmailVerification()) {
            ...
            $verification = new \AdfabUser\Entity\EmailVerification();
    ...
    {
        $mailService = $this->getServiceManager()->get('adfabuser_message');

        $from = $this->getOptions()->getEmailFromAddress();
        $subject = $this->getOptions()->getNewEmailSubjectLine();

        $message = $mailService->createHtmlMessage($from, $to, $subject, 'adfab-user/admin/email/newemail', array('email' => $to, 'password' => $password));
    ...
    public function updateAddress(array $data, $user)
    {
        ...
        $form  = $this->getServiceManager()->get('adfabuser_address_form');
        $form->bind($user);
        $form->setData($data);
    ...
    public function updateNewsletter(array $data)
    {
        ...
        $form  = $this->getServiceManager()->get('adfabuser_newsletter_form');
        $form->bind($user);
        $form->setData($data);
    ...
    public function remove(\AdfabUser\Entity\EmailVerification $m)
    {
        return $this->getEmailVerificationMapper()->remove($m);
    }

    public function sendVerificationEmailMessage(\AdfabUser\Entity\EmailVerification $record)
    {
        $mailService = $this->getServiceManager()->get('adfabuser_message');
    ...
    public function getEmailVerificationMapper()
    {
        if (null === $this->emailVerificationMapper) {
            $this->emailVerificationMapper = $this->getServiceManager()->get('adfabuser_emailverification_mapper');
        }
    ...
    public function getProviderService()
    {
        if ($this->providerService == null) {
            $this->setProviderService($this->getServiceManager()->get('adfabuser_provider_service'));
        }
